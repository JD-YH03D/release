...
const VIDEO_SOURCES = {
  primary: assetPath("image/sasuke/sasuke-Landscape/background-1.mp4"),
} as const;
...
// CUSTOM HOOKS
// ============================================================
function useScrollPosition(threshold = LAYOUT.HEADER_SCROLL_THRESHOLD): boolean {
...
const SafeImage = memo(({
  src, alt, fallbackText = "?", className, ...props
}: React.ImgHTMLAttributes<HTMLImageElement> & { fallbackText?: string }) => {
  const [err, setErr] = useState(false);
  const fb = `https://placehold.co/48x48/0f172a/60a5fa?text=${encodeURIComponent(fallbackText)}`;
  return <img src={err ? fb : src} alt={alt} className={className} onError={() => setErr(true)} loading="lazy" {...props} />;
});
SafeImage.displayName = "SafeImage";

// ============================================================
// VIDEO BACKGROUND — Fixed untuk GitHub Pages
// ============================================================
const VideoBackground = memo(() => {
  ...
  useEffect(() => {
    ...
    const onCanPlay = () => setVideoState("playing");
    const onError = () => setVideoState("error");

    video.addEventListener("canplay", onCanPlay);
    video.addEventListener("error", onError);

    // Manual play untuk browser ketat
    video.load(); // reload source setelah mount
  ...
  return (
    <div className="fixed inset-0 z-0 overflow-hidden" aria-hidden="true">
      ...
      >
        {/*
          assetPath() secara otomatis menambahkan base URL yang benar:
          - localhost  → /image/sasuke/...background-1.mp4
          - gh-pages   → /Releases-Published/image/sasuke/...background-1.mp4
        */}
      ...
      {/* Gradient fallback */}
      <div
        className={`absolute inset-0 bg-gradient-to-br from-slate-950 via-slate-900 to-blue-950 transition-opacity duration-1000 ${
        ...
        >
          ...
          >
            <div className="bg-slate-900/80 px-6 py-4 border-b border-slate-800 flex justify-between items-center">
              ...
              <button onClick={onClose} className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-slate-800 text-slate-500 hover:text-white transition" aria-label="Close modal">
                <X size={16} />
              </button>
            </div>
                ...
                <table className="w-full text-left text-xs font-mono">
                  ...
                  <tbody className="divide-y divide-slate-800/50 text-slate-300">
                    {KEYBOARD_CONTROLS.map((c, i) => (
                      <motion.tr key={c.key} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.3, delay: i * 0.04 }} className="hover:bg-slate-800/30 transition-colors">
                        <td className="px-4 py-3">
                          <kbd className="inline-flex items-center justify-center w-6 h-5 text-[10px] font-bold text-emerald-400 bg-slate-800/70 border border-slate-700 rounded shadow-[0_2px_0_rgba(0,0,0,0.3)]">{c.key}</kbd>
                        </td>
                        <td className={`px-4 py-3 text-xs font-medium ${c.isDiscord ? "text-[#5865F2]" : c.isPrimary ? "text-emerald-400" : "text-white"}`}>{c.label}</td>
            ...
            <div className="px-6 py-4 bg-slate-900/30 border-t border-slate-800 flex justify-end">
              <button onClick={onClose} className="px-6 py-2 bg-slate-800 hover:bg-slate-700 text-white rounded-lg text-xs font-bold transition">Close</button>
            </div>
          </motion.div>
        </motion.div>
      )}
...
const Header = memo(({ onOpenGuide }: { onOpenGuide: () => void }) => {
  ...
  const { isOpen: isMobileOpen, toggle, close: closeMobile } = useMobileMenu();
  ...
  return (
    <motion.header
      className={`fixed top-0 left-0 right-0 z-50 border-b transition-all duration-300 ${isScrolled ? "bg-slate-950/80 backdrop-blur-xl border-slate-800/50 shadow-lg" : "bg-slate-950/50 border-slate-800/30"}`}
      initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}
    >
      <div className="container mx-auto px-6 py-4 flex justify-between items-center">
        <a href="#" className="flex items-center space-x-4 group" onClick={(e) => { e.preventDefault(); scrollTo("#"); }} aria-label="Home">
          <div className="relative">
            ...
            <div className="relative h-10 w-10 rounded-xl overflow-hidden border border-slate-700 bg-slate-900">
              <SafeImage src="https://raw.githubusercontent.com/JD-YH03D/Releases-Published/main/public/image/hero1.png" alt="Logo" fallbackText="JD" className="w-full h-full object-cover" />
            </div>
          </div>
        ...
        <nav className="hidden md:flex items-center space-x-8 text-xs font-semibold uppercase tracking-wider" aria-label="Main navigation">
          {[
            { href: "#directory", label: "Directory", hover: "hover:text-blue-400" },
            { href: "#documentation", label: "Docs", hover: "hover:text-purple-400" },
            { href: "https://github.com/JD-YH03D/Releases-Published/issues", label: "Issues", hover: "hover:text-red-400", external: true },
          ].map((item, i) => (
            <motion.a key={item.href} href={item.href}
              className={`transition-colors duration-200 text-slate-400 ${item.hover}`}
              onClick={(e) => { if (!item.external) { e.preventDefault(); scrollTo(item.href); } }}
              target={item.external ? "_blank" : undefined}
              rel={item.external ? "noopener noreferrer" : undefined}
              initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: i * 0.1 }}
            >
              {item.label}
            </motion.a>
          ))}
          <motion.a href="https://github.com/JD-YH03D/Releases-Published" target="_blank" rel="noopener noreferrer"
          ...
          >
            <GitHubIcon size={14} className="mr-2" /> GitHub
          </motion.a>
        </nav>

        <button className="md:hidden text-slate-400 hover:text-white w-9 h-9 flex items-center justify-center rounded-lg hover:bg-slate-800 transition"
          aria-label={isMobileOpen ? "Close menu" : "Open menu"} aria-expanded={isMobileOpen} onClick={toggle}
        >
            ...
            <nav className="flex flex-col py-4 px-6 space-y-1" aria-label="Mobile navigation">
              {mobileItems.map((item) => (
                <a key={item.href} href={item.href}
                  ...
                  onClick={(e) => { if (!item.external) { e.preventDefault(); scrollTo(item.href, closeMobile); } else closeMobile(); }}
                  target={item.external ? "_blank" : undefined} rel={item.external ? "noopener noreferrer" : undefined}
                >
                  <item.Icon size={16} className={`mr-3 ${item.color}`} /> {item.label}
                </a>
              ))}
            </nav>
          </motion.div>
...
const PulseBadge = memo(() => (
  <motion.div className="inline-flex items-center space-x-2 bg-blue-500/10 border border-blue-500/20 px-3 py-1.5 rounded-full mb-6"
...
const HeroVisual = memo(() => (
  <motion.div className="flex-1 w-full max-w-lg hidden lg:block" aria-hidden="true"
    ...
    <div className="relative group">
      ...
      <div className="relative bg-slate-950/90 border border-slate-800 p-6 rounded-2xl shadow-2xl">
        ...
        <div className="space-y-4">
          {([["75%", 0], ["100%", 0.2], ["83.33%", 0.3], ["66.67%", 0.4]] as const).map(([w, d], i) => (
            <motion.div key={i} className={`h-2.5 rounded-full ${i % 2 === 0 ? "bg-slate-800/70" : "bg-slate-800/50"}`}
              initial={{ width: 0 }} animate={{ width: w }} transition={{ duration: 1, ease: "easeOut", delay: d }} />
          ))}
...
const HeroSection = memo(({ onScrollToDirectory, onScrollToDocs }: { onScrollToDirectory: () => void; onScrollToDocs: () => void }) => (
  <section className="py-16 md:py-24 flex flex-col md:flex-row items-center gap-12 md:gap-16">
    <motion.div className="flex-1 text-center md:text-left" initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.8 }}>
      <PulseBadge />
      <motion.h1 className="text-4xl md:text-6xl font-black mb-6 leading-[1.1] tracking-tight bg-gradient-to-r from-white via-slate-200 to-slate-400 bg-clip-text text-transparent"
        initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.3 }}
      >
        Web Automation<br /><span className="text-blue-500">Script Engine</span>
      </motion.h1>
      <motion.p className="text-slate-400 text-base md:text-lg mb-10 max-w-xl leading-relaxed mx-auto md:mx-0"
      ...
      <motion.div className="flex flex-wrap gap-4 justify-center md:justify-start" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.6 }}>
        <motion.button onClick={onScrollToDirectory} className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-8 py-3.5 rounded-xl font-bold shadow-lg shadow-blue-500/35 flex items-center cursor-pointer" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
          Explore Scripts <ChevronRight size={16} className="ml-3 opacity-70" />
        </motion.button>
        <motion.button onClick={onScrollToDocs} className="bg-slate-900/50 hover:bg-slate-800 text-slate-300 px-8 py-3.5 rounded-xl font-bold transition border border-slate-800" whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}>
          Documentation
        </motion.button>
      </motion.div>
    </motion.div>
...
const ScriptCardItem = memo(({ card, onOpenGuide }: { card: ScriptCard; onOpenGuide?: () => void }) => (
  <motion.article className={`rounded-2xl overflow-hidden group border bg-slate-800/30 backdrop-blur-xl border-slate-700/50 ${card.accentHover} transition-all`}
    ...
    <div className="bg-slate-900/60 px-6 py-5 border-b border-slate-800/50 flex flex-col sm:flex-row items-center justify-between gap-4">
      ...
      <div className="flex items-center space-x-4">
        {card.hasGuide && onOpenGuide && (
          <button onClick={onOpenGuide} className="text-xs font-bold text-emerald-400 hover:bg-emerald-400/10 px-4 py-2 rounded-lg transition border border-emerald-400/20 flex items-center">
            <Book size={14} className="mr-2" /> Guide
          </button>
        )}
      ...
      <table className="w-full text-left text-sm">
        ...
        <tbody className="divide-y divide-slate-800/30">
          {card.assets.map((asset, i) => (
            <motion.tr key={asset.version} className={`${card.hoverRowBg} transition-colors`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.1 }}>
...
const TabFilter = memo(({ activeTab, onSwitch }: { activeTab: ActiveTab; onSwitch: (t: ActiveTab) => void }) => (
  <div className="flex bg-slate-900/50 p-1.5 rounded-xl border border-slate-800 w-fit" role="tablist">
    {(["production", "legacy"] as ActiveTab[]).map((tab) => (
      <motion.button key={tab} role="tab" aria-selected={activeTab === tab} onClick={() => onSwitch(tab)}
        className={`px-5 py-2 text-xs font-bold rounded-lg transition-all capitalize ${activeTab === tab ? "text-white bg-blue-600 shadow-lg" : "text-slate-500 hover:text-slate-300"}`}
...
const DirectorySection = memo(({ onOpenGuide }: { onOpenGuide: () => void }) => {
  ...
  return (
    <section id="directory" className="py-20 border-t border-slate-900 scroll-mt-24">
      <motion.div className="flex flex-col md:flex-row md:items-center justify-between mb-12 gap-6"
        ...
        >
          {activeTab === "production" ? (
            <div className="grid grid-cols-1 gap-10">
              ...
              <motion.div className="border-2 border-dashed border-slate-800/60 p-12 rounded-2xl flex flex-col items-center justify-center text-center group hover:border-slate-700 transition-colors"
            ...
            <div className="py-20 flex flex-col items-center justify-center text-center">
              <div className="h-16 w-16 bg-slate-900 border border-slate-800 rounded-2xl flex items-center justify-center mb-5 text-slate-600"><FolderOpen size={24} /></div>
              <p className="text-slate-400 text-base font-bold">Legacy Archive</p>
              <p className="text-slate-600 text-sm mt-2 max-w-xs">Older script versions are not publicly available.</p>
            </div>
          )}
        </motion.div>
      </AnimatePresence>
...
const TerminalPreview = memo(() => (
  <motion.div className="bg-[#0b1120] rounded-2xl border border-slate-800 overflow-hidden shadow-2xl"
    initial={{ opacity: 0, x: 40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}
  >
    <div className="flex items-center justify-between px-5 py-3.5 bg-slate-900/80 border-b border-slate-800">
      ...
      <div className="flex items-center text-[10px] font-mono text-slate-500 font-bold uppercase tracking-widest">
        <Terminal size={12} className="mr-2" /> development_shell
      </div>
    </div>
    <div className="p-6 text-[12px] font-mono leading-relaxed overflow-x-auto">
      {[
        { n: 1, c: <span className="text-slate-500"># Clone the repository</span>, mt: false },
        { n: 2, c: <span className="text-blue-400">git clone <span className="text-emerald-400">&quot;https://github.com/JD-YH03D/Scripts&quot;</span></span>, mt: false },
        { n: 3, c: <span className="text-blue-400">cd <span className="text-slate-300">Scripts/build</span></span>, mt: false },
        { n: 4, c: <span className="text-slate-500"># Install dependencies and optimize</span>, mt: true },
        { n: 5, c: <span className="text-blue-400">npm <span className="text-slate-300">install</span> && npm <span className="text-slate-300">run optimize</span></span>, mt: false },
        { n: 6, c: <span className="text-emerald-400 font-bold">✓ Build success: <span className="text-slate-300 font-normal">2 packages optimized.</span></span>, mt: true },
      ].map(({ n, c, mt }) => (
        <div key={n} className={`flex gap-4 mb-1 ${mt ? "mt-4" : ""}`}>
          <span className="text-slate-700 select-none w-4 text-right flex-shrink-0">{n}</span>
          <p>{c}</p>
        </div>
      ))}
    </div>
...
const DocumentationSection = memo(() => (
  <section id="documentation" className="py-24 border-t border-slate-900 scroll-mt-24">
    <div className="grid lg:grid-cols-2 gap-16 items-start">
      <motion.div className="space-y-8" initial={{ opacity: 0, x: -40 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
        <div>
          <h2 className="text-3xl font-black text-white mb-4 tracking-tight">Standardized Architecture</h2>
          <p className="text-slate-400 leading-relaxed">
            All scripts conform to the <span className="text-blue-400 font-bold">Standardized Script Schema (S3)</span>. We prioritize code security, runtime performance, and a smooth user experience.
          </p>
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
          {DOC_FEATURES.map((item, i) => (
            <motion.div key={item.title} className="p-5 rounded-2xl border border-slate-800 hover:border-slate-700 transition-colors"
        ...
        <motion.div className="pt-6 border-t border-slate-900" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }}>
          <a href="https://github.com/JD-YH03D/Releases-Published/issues" target="_blank" rel="noopener noreferrer" className="group flex items-center space-x-3 w-fit">
...
const Footer = memo(() => {
  const scrollTo = useScrollToSection();
  return (
    <motion.footer className="py-14 border-t border-slate-900 bg-slate-950/90 mt-12 relative z-10"
      ...
      <div className="container mx-auto px-6 max-w-6xl">
        <div className="flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="text-center md:text-left">
            ...
            <p className="text-slate-700 text-[10px] mt-1 uppercase tracking-tight">Production-Grade Web Automation Solutions.</p>
          </div>
          <div className="flex flex-col items-center md:items-end gap-4">
            <div className="flex items-center space-x-6">
              <a href="#directory" onClick={(e) => { e.preventDefault(); scrollTo("#directory"); }} className="text-slate-500 hover:text-white transition-colors text-xs font-bold uppercase tracking-widest">Scripts</a>
              <a href="#documentation" onClick={(e) => { e.preventDefault(); scrollTo("#documentation"); }} className="text-slate-500 hover:text-white transition-colors text-xs font-bold uppercase tracking-widest">Docs</a>
...
// ERROR BOUNDARY
// ============================================================
interface EFProps { error: Error; resetErrorBoundary: () => void; }

const ErrorFallback = memo(({ error, resetErrorBoundary }: EFProps) => (
  <div className="min-h-screen flex items-center justify-center bg-slate-950">
    <div className="text-center p-8">
      <div className="h-16 w-16 bg-red-500/10 border border-red-500/20 rounded-2xl flex items-center justify-center mx-auto mb-6"><X size={28} className="text-red-400" /></div>
      ...
      <button onClick={resetErrorBoundary} className="px-6 py-2.5 bg-blue-600 hover:bg-blue-500 text-white rounded-lg font-bold transition text-sm">Try Again</button>
    </div>
  </div>
));
...
class ErrorBoundary extends React.Component<
  { children: React.ReactNode; FallbackComponent: React.ComponentType<EFProps> },
